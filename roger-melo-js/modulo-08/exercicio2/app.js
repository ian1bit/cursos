...
const numbersInAcendingOrder = getArrayCopy(numbers)
  .sort((item2, item1) => item2 - item1)

console.log(numbersInAcendingOrder)
...
const cookedIgredients = ingredients.reduce((acc, item, index, array) => {
  const currentWordGender = /a$/.test(item) ? 'cozida' : 'cozido'
  const isLastItem = index === array.length - 1
  const ingredientMessage = acc + `${item} ${currentWordGender}`

  return isLastItem ? ingredientMessage : `${ingredientMessage}, `
}, '')

console.log(cookedIgredients)
